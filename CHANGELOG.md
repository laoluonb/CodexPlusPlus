...
- 修复 API Key 登录并开启插件市场解锁后，打开插件详情页显示“无法加载插件”的问题。
- 当远程插件目录拒绝 API Key 读取 `plugin/read` 详情时，改为从本地插件市场快照返回 Codex 所需的插件详情数据，不再把插件列表误用作详情响应。

## 1.2.58 - 2026-08-25

- 修复 API Key 登录并开启插件市场解锁后，插件列表可见但安装失败的问题。
- 本地插件市场快照与前端合并结果现在会为每个插件传递真实的 `marketplace.json` 文件路径，安装请求仍由 Codex 原生接口处理。

## 1.2.57 - 2026-08-25

- 修复 Codex++ 内嵌页面越过原生顶栏的问题，页面现在跟随 Codex 工程主内容区的实际边界，并在侧栏或窗口尺寸变化后自动重新定位。
- 修复开启插件市场解锁后无法安装插件的问题；解锁补丁只扩展插件列表，安装请求保持 Codex 原生参数不变。

## 1.2.56 - 2026-08-25

- 修复安装维护中保存 Codex Store 路径后，Store 升级残留旧目录导致皮肤运行诊断误报“官方应用身份失败”的问题。
- 保存的 Store 路径现在会自动跟随系统当前注册版本，运行期间发生 Store 更新也会重新读取包身份。

## 1.2.55 - 2026-08-25

- DreamSkin 社区与主题市场改为分页展示，每页最多 12 个主题。
- 搜索或切换排序后自动返回第一页，远程主题数量变化时自动修正当前页码。

## 1.2.54 - 2026-08-25

- 同步 BigPizzaV3/CodexPlusPlus 上游的 10 个提交，包括启动稳定性、DeepSeek 工具历史和供应商配置页改进。
- 修复 Dream Skin v1.5.14 已正常生效但实机验证仍显示失败的问题。
- Windows 应用自动探测优先使用系统当前注册的 Store 包，避免残留安装目录导致身份误报。
- 继续保持无广告、无赞助入口、无赞助图片、无宣传站点及无推广预设。

## 1.2.53 - 2026-08-24

- 皮肤切换改用 Codex-Dream-Skin v1.5.14 的原版 renderer 与 CSS，并适配当前 Codex 首页结构。
- 修复首页背景层遮挡标题、项目选择器和输入框的问题。
- GitHub Release 更新检测、项目主页和问题反馈地址改为 `laoluonb/CodexPlusPlus`。
- 移除推荐内容、赞助入口、赞助图片、社群推广入口和宣传站点文件。
- 清理 Dream Skin 主题中的推广字段，旧设置中的 `promoTitle`、`promoSub`、`promoUrl` 会自动移除。
